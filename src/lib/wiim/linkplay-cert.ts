/**
 * LinkPlay mutual-TLS client certificate.
 *
 * Newer WiiM firmware (e.g. WiiM Ultra on fw 5.x) negotiates HTTPS with a
 * client certificate. This is the well-known shared LinkPlay client cert that
 * the reference open-source library `python-linkplay` and Home Assistant use to
 * talk to the device on the LAN — it is NOT a user secret. Most older units
 * accept a plain TLS handshake with verification disabled and ignore this.
 *
 * Source: https://github.com/Velleman/python-linkplay (src/linkplay/consts.py)
 *
 * Can be overridden at runtime via WIIM_CLIENT_CERT_PATH / WIIM_CLIENT_KEY_PATH.
 */

export const LINKPLAY_CLIENT_KEY = `-----BEGIN PRIVATE KEY-----

/NDvel7IF63U4UEkrVVt1bYg
-----END PRIVATE KEY-----
`;

export const LINKPLAY_CLIENT_CERT = `-----BEGIN CERTIFICATE-----

uc2m8y8O/hXPSfKd
-----END CERTIFICATE-----
`;
