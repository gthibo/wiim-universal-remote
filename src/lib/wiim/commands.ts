...
/** A paired Bluetooth sink (speaker/DAC/headphone) that output can switch to. */
export interface BtSink {
  name: string;
  mac: string; // the `ad` field from getbthistory
}

/**
 * Paired Bluetooth SINKS (Audio Sink role only) from getbthistory. These are
 * the devices output can be routed to via connectBtSink. Audio *Source* entries
 * (phones casting TO the WiiM) are excluded. Verified on Ultra against a Topping
 * D70 Pro. Returns [] on failure or if the command is unsupported.
 */
export async function fetchBtSinks(ip: string): Promise<BtSink[]> {
  try {
    const text = await send(ip, Cmd.btHistory, 5000);
    const raw = safeJson<{ list?: { name?: unknown; ad?: unknown; role?: unknown }[] }>(text);
    const list = raw?.list;
    if (!Array.isArray(list)) return [];
    const out: BtSink[] = [];
    for (const e of list) {
      if (e?.role !== "Audio Sink") continue; // only connectable output devices
      const mac = typeof e.ad === "string" ? e.ad.trim() : "";
      const name = typeof e.name === "string" && e.name.trim() ? e.name.trim() : mac;
      if (mac) out.push({ name, mac });
    }
    return out;
  } catch {
    return [];
...
/**
 * Connect a Bluetooth sink by MAC — this switches audio OUTPUT to that device.
 * BT-out is a separate axis from the wired hardware output mode (setOutput):
 * connecting a sink routes audio to it without changing the hardware-mode value.
 * Verified on Ultra (connectbta2dpsynk → "OK").
 */
export async function connectBtSink(ip: string, mac: string): Promise<void> {
  const command = Cmd.btConnect(mac);
  const text = await send(ip, command);
  // Device replies "OK" on success, "Failed" if the sink can't be reached.
  if (text.trim().toLowerCase().startsWith("failed")) {
    throw new WiimError(`Could not connect BT sink ${mac}`, "BT_CONNECT_FAILED");
  }
  assertAccepted(text, command);
}
